<?php
// modules/mantenimiento/agregar.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        $stmt = $conn->prepare("INSERT INTO tareas_mantenimiento (id_habitacion, descripcion, fecha_inicio, fecha_fin, estado) VALUES (?, ?, ?, ?, ?)");
        $stmt->execute([$id_habitacion, $descripcion, $fecha_inicio, $fecha_fin ?: null, $estado]);
...
<div class="habitaciones-container">
    <div class="habitaciones-header">
        <h1>Nueva Tarea de Mantenimiento</h1>
        ...
        <form action="" method="post">
            ...
            <select name="id_habitacion" id="id_habitacion" required>
                ...
                <?php foreach ($habitaciones as $hab): ?>
                    <option value="<?= $hab['id_habitacion'] ?>"><?= htmlspecialchars($hab['numero']) ?></option>
                <?php endforeach; ?>
            </select>
